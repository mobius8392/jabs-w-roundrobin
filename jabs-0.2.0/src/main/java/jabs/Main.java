...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Simulate Round Robin network of 40 nodes for 1 hour
        scenario = new RoundRobinGlobalNetworkScenario("One day in the life of RoundRobin", 1,
                1000, 600, 6);
        scenario.AddNewLogger(new BlockchainReorgLogger(Paths.get("_output/round robin-reorgs-log.csv")));
        scenario.AddNewLogger(new BlockConfirmationLogger(Paths.get("_output/round robin-confirmations-log.csv")));
        scenario.AddNewLogger(new BlockDeliveryLogger(Paths.get("_output/round robin-block-delivery-log.csv")));
        scenario.AddNewLogger(new BlockPropagationDelayLogger(
                Paths.get("_output/round robin-50-propagation-delay-log.csv"),0.5));
        scenario.AddNewLogger(new BlockPropagationDelayLogger(
                Paths.get("_output/round robin-90-propagation-delay-log.csv"),0.9));
        scenario.run();
    }
}
